...
工具调用具有幂等键，同一个会议、行动项和工具重复执行时会返回既有对象，不会重复创建。所有提取、审批和执行事件写入 SQLite 审计日志。默认使用规则基线；配置 OpenAI-compatible 端点后可由模型结构化抽取，但来源原句必须逐字命中会议记录。
...
如需模型辅助抽取，按 `.env.example` 设置三个 `AGENT_LLM_*` 环境变量后再启动后端；不要提交真实 Key。
